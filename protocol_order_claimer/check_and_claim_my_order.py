...
while True:
    ...
    totalUnclaimedOrders = FugaziViewer.functions.getUnclaimedOrdersLength().call({"from": acc.address})
    print(
        f"Total outstanding orders are: {totalUnclaimedOrders}"
    )

    if totalUnclaimedOrders > 0:
        # get the info about the oldest unclaimed order
        oldestUnclaimedOrder = FugaziViewer.functions.getUnclaimedOrder(
            0
        ...
        if poolInfo[0] > oldestUnclaimedOrder[1]:
            ...
            try:
                # claim the order
                tx = FugaziPoolAction.functions.claim(oldestUnclaimedOrder[0], oldestUnclaimedOrder[1]).build_transaction(
                ...
                print(f"Error: {e}")
        elif poolInfo[0] == oldestUnclaimedOrder[1] and int(datetime.datetime.now().timestamp()) > poolInfo[1] + 30:
            print("Order is claimable but you should settle the batch first")
            
            try:
                # settle the batch
                tx = FugaziPoolAction.functions.settleBatch(oldestUnclaimedOrder[0]).build_transaction(
                    {"from": acc.address, "nonce": w3.eth.get_transaction_count(acc.address)}
                )
                signed_tx = acc.sign_transaction(tx)
                tx_hash = w3.eth.send_raw_transaction(signed_tx.rawTransaction)
